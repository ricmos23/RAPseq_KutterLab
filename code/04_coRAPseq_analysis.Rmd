...
```{r load data}
# RAP data
HUR <- read.table("./Data/Benchmarking/RAPseq/annotated/hsHuRHUMAN.peaks.txt", 
                  stringsAsFactors = F, header = T)
PTBP1 <- read.table("./Data/Benchmarking/RAPseq/annotated/PTBP1.peaks.txt", 
                    stringsAsFactors = F, header = T)
HUR_PTBP1 <- read.table("./Data/Benchmarking/RAPseq/annotated/HuRPTBP1.peaks.txt", 
                        stringsAsFactors = F, header = T)
print(nrow(HUR))
print(nrow(PTBP1))
...
pw <- wilcox.test(PTBP1_HURPTBP1_overlaps_adjust, HUR_PTBP1_PTBP1HUR_overlaps_adjust, alternative = "less")
pwp <- pw$p.value
if ( pwp <= 0.001 ) {
  to_add <- "***" 
} else if ( btp <= 0.01 ) {
...
pw <- wilcox.test(HUR_HURPTBP1_overlaps_adjust, HUR_PTBP1_HURPTBP1_overlaps_adjust, alternative = "less")
pwp <- pw$p.value
if ( pwp <= 0.001 ) {
  to_add <- "***" 
} else if ( btp <= 0.01 ) {
...
# write.csv(GOs, "./Data/Benchmarking/GOs.csv")
