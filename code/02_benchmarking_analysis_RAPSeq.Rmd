...
wilcox.test(data_cors[data_cors$assay == "aRAP",]$cors, data_cors[data_cors$assay == "eCLIP",]$cors)
...
for (i in 1:length(Names_protein)){
  ...
  venns[[i]] <- plot( v, fills=c(alpha(acqua_greens[i*2],0.7),alpha(greys[i],0.7), yellows[i*2]), main=Names_protein[i], quantities=TRUE, edges=F )
}
...
RAPs_CLIPs_common_HNRNPA1$intersection <- rep("CLIP_common",nrow(RAPs_CLIPs_common_HNRNPA1))
...
CLIPs_uniq_HNRNPA1$intersection <- rep("CLIP_uniq",nrow(CLIPs_uniq_HNRNPA1))
...
plot_HNRNPA1
HNRNPA1_pval <- wilcox.test(RAPs_CLIPs_common_HNRNPA1$Gene_BS_CLIP,CLIPs_uniq_HNRNPA1$Gene_BS_CLIP)$p.value
...
RAPs_CLIPs_common_HNRNPC$intersection <- rep("CLIP_common",nrow(RAPs_CLIPs_common_HNRNPC))
CLIPs_uniq_HNRNPC <- CLIPs_HNRNPC[grep(paste(setdiff(CLIPs_HNRNPC$gene_ID,RAPs_HNRNPC$gene_ID),collapse="|"),CLIPs_HNRNPC$gene_ID),]
CLIPs_uniq_HNRNPC$intersection <- rep("CLIP_uniq",nrow(CLIPs_uniq_HNRNPC))
...
plot_HNRNPC <- ggplot( data = CLIPS_common_uniq_HNRNPC, aes( x=intersection,y=log2(Gene_BS_CLIP) ) ) +
  ...
  coord_cartesian(ylim=c(0,13))
plot_HNRNPC
HNRNPC_pval <- wilcox.test(RAPs_CLIPs_common_HNRNPC$Gene_BS_CLIP,CLIPs_uniq_HNRNPC$Gene_BS_CLIP)$p.value 
...
RAPs_CLIPs_common_PTBP1$intersection <- rep("CLIP_common",nrow(RAPs_CLIPs_common_PTBP1))
CLIPs_uniq_PTBP1 <- CLIPs_PTBP1[grep(paste(setdiff(CLIPs_PTBP1$gene_ID,RAPs_PTBP1$gene_ID),collapse="|"),CLIPs_PTBP1$gene_ID),]
CLIPs_uniq_PTBP1$intersection <- rep("CLIP_uniq",nrow(CLIPs_uniq_PTBP1))
...
plot_PTBP1 <- ggplot( data = CLIPS_common_uniq_PTBP1, aes( x=intersection,y=log2(Gene_BS_CLIP) ) ) +
  ...
  coord_cartesian(ylim=c(0,13))
plot_PTBP1
PTBP1_pval <- wilcox.test(RAPs_CLIPs_common_PTBP1$Gene_BS_CLIP,CLIPs_uniq_PTBP1$Gene_BS_CLIP)$p.value
...
RAPs_CLIPs_common_RBFOX2$intersection <- rep("CLIP_common",nrow(RAPs_CLIPs_common_RBFOX2))
CLIPs_uniq_RBFOX2 <- CLIPs_RBFOX2[grep(paste(setdiff(CLIPs_RBFOX2$gene_ID,RAPs_RBFOX2$gene_ID),collapse="|"),CLIPs_RBFOX2$gene_ID),]
CLIPs_uniq_RBFOX2$intersection <- rep("CLIP_uniq",nrow(CLIPs_uniq_RBFOX2))
...
plot_RBFOX2 <- ggplot( data = CLIPS_common_uniq_RBFOX2, aes( x=intersection,y=log2(Gene_BS_CLIP) ) ) +
  ...
  coord_cartesian(ylim=c(0,13))
plot_RBFOX2
RBFOX2_pval <- wilcox.test(RAPs_CLIPs_common_RBFOX2$Gene_BS_CLIP,CLIPs_uniq_RBFOX2$Gene_BS_CLIP)$p.value
...
RAPs_CLIPs_common_YBX3$intersection <- rep("CLIP_common",nrow(RAPs_CLIPs_common_YBX3))
CLIPs_uniq_YBX3 <- CLIPs_YBX3[grep(paste(setdiff(CLIPs_YBX3$gene_ID,RAPs_YBX3$gene_ID),collapse="|"),CLIPs_YBX3$gene_ID),]
CLIPs_uniq_YBX3$intersection <- rep("CLIP_uniq",nrow(CLIPs_uniq_YBX3))
...
plot_YBX3 <- ggplot( data = CLIPS_common_uniq_YBX3, aes( x=intersection,y=log2(Gene_BS_CLIP) ) ) +
  ...
  coord_cartesian(ylim=c(0,13))
plot_YBX3
YBX3_pval <- wilcox.test(RAPs_CLIPs_common_YBX3$Gene_BS_CLIP,CLIPs_uniq_YBX3$Gene_BS_CLIP)$p.value
...
CLIPS_common_uniq_HNRNPA1 <- CLIPS_common_uniq_HNRNPA1[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
CLIPS_common_uniq_HNRNPC <- CLIPS_common_uniq_HNRNPC[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
CLIPS_common_uniq_PTBP1 <- CLIPS_common_uniq_PTBP1[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
CLIPS_common_uniq_RBFOX2 <- CLIPS_common_uniq_RBFOX2[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
CLIPS_common_uniq_YBX3 <- CLIPS_common_uniq_YBX3[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
```

```{r figure2GCommonComparision}
# HNRNPA1
HNRNPA1_CLIPS_RAPS_Qun <- merge(RAPs_HNRNPA1, CLIPs_HNRNPA1, by = "gene_ID")
colnames(HNRNPA1_CLIPS_RAPS_Qun) <- c("gene_ID", "RAP_common", "Gene_BS_CLIP")
HNRNPA1_CLIPS_RAPS_Qun_long <- pivot_longer(
  HNRNPA1_CLIPS_RAPS_Qun, 
  cols = -gene_ID,
  names_to = "Methods", 
  values_to = "BS"
)
#CLIPS_common_uniq_HNRNPA1 <- CLIPS_common_uniq_HNRNPA1[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
colnames(CLIPS_common_uniq_HNRNPA1) <- c("gene_ID", "Methods", "BS")
data_HNRNPA1_comparision_Qun <- rbind(HNRNPA1_CLIPS_RAPS_Qun_long, CLIPS_common_uniq_HNRNPA1)
data_HNRNPA1_comparision_Qun <- data_HNRNPA1_comparision_Qun[data_HNRNPA1_comparision_Qun$Methods != "Gene_BS_CLIP",]
data_HNRNPA1_comparision_Qun$Methods <- factor(data_HNRNPA1_comparision_Qun$Methods, levels = c("RAP_common", "CLIP_common", "CLIP_uniq"))
plot_HNRNPA1 <- ggplot( data = data_HNRNPA1_comparision_Qun, aes( x=Methods,y=log2(BS) ) ) +
  geom_violin(aes(fill = Methods), bw=0.85, width=0.8) + 
  #scale_fill_manual(values = c(yellows[2],greys[5])) +
  geom_boxplot(color="black", fill=NA, width = 0.2, outlier.shape = NA) + 
  theme_pubr() +
  coord_cartesian(ylim=c(0,13))
plot_HNRNPA1
HNRNPA1_pval <- wilcox.test(HNRNPA1_CLIPS_RAPS_Qun$RAP_common,HNRNPA1_CLIPS_RAPS_Qun$Gene_BS_CLIP)$p.value

# HNRNPC
HNRNPC_CLIPS_RAPS_Qun <- merge(RAPs_HNRNPC, CLIPs_HNRNPC, by = "gene_ID")
colnames(HNRNPC_CLIPS_RAPS_Qun) <- c("gene_ID", "RAP_common", "Gene_BS_CLIP")
HNRNPC_CLIPS_RAPS_Qun_long <- pivot_longer(
  HNRNPC_CLIPS_RAPS_Qun, 
  cols = -gene_ID,
  names_to = "Methods", 
  values_to = "BS"
)
#CLIPS_common_uniq_HNRNPC <- CLIPS_common_uniq_HNRNPC[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
colnames(CLIPS_common_uniq_HNRNPC) <- c("gene_ID", "Methods", "BS")
data_HNRNPC_comparision_Qun <- rbind(HNRNPC_CLIPS_RAPS_Qun_long, CLIPS_common_uniq_HNRNPC)
data_HNRNPC_comparision_Qun <- data_HNRNPC_comparision_Qun[data_HNRNPC_comparision_Qun$Methods != "Gene_BS_CLIP",]
data_HNRNPC_comparision_Qun$Methods <- factor(data_HNRNPC_comparision_Qun$Methods, levels = c("RAP_common", "CLIP_common", "CLIP_uniq"))
plot_HNRNPC <- ggplot( data = data_HNRNPC_comparision_Qun, aes( x=Methods,y=log2(BS) ) ) +
  geom_violin(aes(fill = Methods), bw=0.85, width=0.8) + 
  #scale_fill_manual(values = c(yellows[2],greys[5])) +
  geom_boxplot(color="black", fill=NA, width = 0.2, outlier.shape = NA) + 
  theme_pubr() +
  coord_cartesian(ylim=c(0,13))
plot_HNRNPC
HNRNPC_pval <- wilcox.test(HNRNPC_CLIPS_RAPS_Qun$RAP_common,HNRNPC_CLIPS_RAPS_Qun$Gene_BS_CLIP)$p.value

# PTBP1
PTBP1_CLIPS_RAPS_Qun <- merge(RAPs_PTBP1, CLIPs_PTBP1, by = "gene_ID")
colnames(PTBP1_CLIPS_RAPS_Qun) <- c("gene_ID", "RAP_common", "Gene_BS_CLIP")
PTBP1_CLIPS_RAPS_Qun_long <- pivot_longer(
  PTBP1_CLIPS_RAPS_Qun, 
  cols = -gene_ID,
  names_to = "Methods", 
  values_to = "BS"
)
#CLIPS_common_uniq_PTBP1 <- CLIPS_common_uniq_PTBP1[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
colnames(CLIPS_common_uniq_PTBP1) <- c("gene_ID", "Methods", "BS")
data_PTBP1_comparision_Qun <- rbind(PTBP1_CLIPS_RAPS_Qun_long, CLIPS_common_uniq_PTBP1)
data_PTBP1_comparision_Qun <- data_PTBP1_comparision_Qun[data_PTBP1_comparision_Qun$Methods != "Gene_BS_CLIP",]
data_PTBP1_comparision_Qun$Methods <- factor(data_PTBP1_comparision_Qun$Methods, levels = c("RAP_common", "CLIP_common", "CLIP_uniq"))
plot_PTBP1 <- ggplot( data = data_PTBP1_comparision_Qun, aes( x=Methods,y=log2(BS) ) ) +
  geom_violin(aes(fill = Methods), bw=0.85, width=0.8) + 
  #scale_fill_manual(values = c(yellows[2],greys[5])) +
  geom_boxplot(color="black", fill=NA, width = 0.2, outlier.shape = NA) + 
  theme_pubr() +
  coord_cartesian(ylim=c(0,13))
plot_PTBP1
PTBP1_pval <- wilcox.test(PTBP1_CLIPS_RAPS_Qun$RAP_common,PTBP1_CLIPS_RAPS_Qun$Gene_BS_CLIP)$p.value

# RBFOX2
RBFOX2_CLIPS_RAPS_Qun <- merge(RAPs_RBFOX2, CLIPs_RBFOX2, by = "gene_ID")
colnames(RBFOX2_CLIPS_RAPS_Qun) <- c("gene_ID", "RAP_common", "Gene_BS_CLIP")
RBFOX2_CLIPS_RAPS_Qun_long <- pivot_longer(
  RBFOX2_CLIPS_RAPS_Qun, 
  cols = -gene_ID,
  names_to = "Methods", 
  values_to = "BS"
)
#CLIPS_common_uniq_RBFOX2 <- CLIPS_common_uniq_RBFOX2[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
colnames(CLIPS_common_uniq_RBFOX2) <- c("gene_ID", "Methods", "BS")
data_RBFOX2_comparision_Qun <- rbind(RBFOX2_CLIPS_RAPS_Qun_long, CLIPS_common_uniq_RBFOX2)
data_RBFOX2_comparision_Qun <- data_RBFOX2_comparision_Qun[data_RBFOX2_comparision_Qun$Methods != "Gene_BS_CLIP",]
data_RBFOX2_comparision_Qun$Methods <- factor(data_RBFOX2_comparision_Qun$Methods, levels = c("RAP_common", "CLIP_common", "CLIP_uniq"))
plot_RBFOX2 <- ggplot( data = data_RBFOX2_comparision_Qun, aes( x=Methods,y=log2(BS) ) ) +
  geom_violin(aes(fill = Methods), bw=0.85, width=0.8) + 
  #scale_fill_manual(values = c(yellows[2],greys[5])) +
  geom_boxplot(color="black", fill=NA, width = 0.2, outlier.shape = NA) + 
  theme_pubr() +
  coord_cartesian(ylim=c(0,13))
plot_RBFOX2
RBFOX2_pval <- wilcox.test(RBFOX2_CLIPS_RAPS_Qun$RAP_common,RBFOX2_CLIPS_RAPS_Qun$Gene_BS_CLIP)$p.value

# YBX3
YBX3_CLIPS_RAPS_Qun <- merge(RAPs_YBX3, CLIPs_YBX3, by = "gene_ID")
colnames(YBX3_CLIPS_RAPS_Qun) <- c("gene_ID", "RAP_common", "Gene_BS_CLIP")
YBX3_CLIPS_RAPS_Qun_long <- pivot_longer(
  YBX3_CLIPS_RAPS_Qun, 
  cols = -gene_ID,
  names_to = "Methods", 
  values_to = "BS"
...
#CLIPS_common_uniq_YBX3 <- CLIPS_common_uniq_YBX3[,c("gene_ID", "intersection", "Gene_BS_CLIP")]
colnames(CLIPS_common_uniq_YBX3) <- c("gene_ID", "Methods", "BS")
data_YBX3_comparision_Qun <- rbind(YBX3_CLIPS_RAPS_Qun_long, CLIPS_common_uniq_YBX3)
data_YBX3_comparision_Qun <- data_YBX3_comparision_Qun[data_YBX3_comparision_Qun$Methods != "Gene_BS_CLIP",]
data_YBX3_comparision_Qun$Methods <- factor(data_YBX3_comparision_Qun$Methods, levels = c("RAP_common", "CLIP_common", "CLIP_uniq"))
plot_YBX3 <- ggplot( data = data_YBX3_comparision_Qun, aes( x=Methods,y=log2(BS) ) ) +
  geom_violin(aes(fill = Methods), bw=0.85, width=0.8) + 
  #scale_fill_manual(values = c(yellows[2],greys[5])) +
  geom_boxplot(color="black", fill=NA, width = 0.2, outlier.shape = NA) + 
  theme_pubr() +
  coord_cartesian(ylim=c(0,13))
plot_YBX3
YBX3_pval <- wilcox.test(YBX3_CLIPS_RAPS_Qun$RAP_common,YBX3_CLIPS_RAPS_Qun$Gene_BS_CLIP)$p.value
...
ggsave(filename = "./Figure/Figure2G_Violin_Qun.pdf", plot = p_figure_violin_2G, width = 4.7, height = 12.6)
```
...
for (i in names(GNWYG_scores)){
  ...
  test_compare <- wilcox.test(GCAUG,GNWYG_scores[[i]])$p.value
